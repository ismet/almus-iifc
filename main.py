#!/usr/bin/env python3
"""
Almus Barajı GÜNLÜK Hidrolojik Tahmin Modeli
============================================
Kemerdere günlük modelinin Almus havzasına uyarlanmış sürümü. Hedef: günlük giriş
akımı (m³/s) — E14A018 istasyonu (E14A018_daily_inflow_1999-2025.xlsx, 'Q smoothed').
Girdiler GÜNLÜK:
...
Geometri: watershed (5).kml (havza poligonu) + rivers (2).kml (dereler) — bu klasörde.
Model: HİBRİT
...
Çalıştırma:  streamlit run almus_daily_model.py
"""
...
st.set_page_config(page_title="Almus Barajı Günlük Akım Tahmin Modeli", page_icon="🌊", layout="wide")
...
if not st.session_state.authenticated:
    ...
    with c2:
        st.title("🌊 Almus Barajı")
        st.caption("Günlük Akım Tahmin Modeli")
        with st.form("login"):
            u = st.text_input("Kullanıcı Adı")
            p = st.text_input("Şifre", type="password")
            if st.form_submit_button("Giriş Yap", use_container_width=True):
                accounts = st.secrets.get("accounts", {})
                match = accounts.get(u)
                if match and match["password"] == p:
                    st.session_state.authenticated = True
                    st.session_state.username = u
                    st.session_state.is_admin = match.get("is_admin", False)
                    st.rerun()
                else:
...
DEFAULT_FLOW = os.path.join(SCRIPT_DIR, "E14A018_daily_inflow_1999-2025.xlsx")
# Havza poligonu: 'watershed/havza/basin' içeren ilk .kml/.kmz (örn. watershed (5).kml), yoksa None
DEFAULT_KMZ = next((os.path.join(SCRIPT_DIR, _f) for _f in (os.listdir(SCRIPT_DIR) if os.path.isdir(SCRIPT_DIR) else [])
                    if _f.lower().endswith((".kmz", ".kml"))
                    and any(k in _f.lower() for k in ("watershed", "havza", "basin"))), None)
...
def points_to_kmz_bytes(bands, basin_area_km2=None):
    ...
    parts = ['<?xml version="1.0" encoding="UTF-8"?>', '<kml xmlns="http://www.opengis.net/kml/2.2">',
             '<Document>', '<name>Almus Model Noktaları</name>']
    for b in bands:
...
def load_daily_flow(source, sheet=None, date_col=None, q_col=None, unit_factor=1.0/86400.0, start=None):
    ...
    if q_col is None or q_col not in df.columns:
        # Önce 'toplam su', sonra yumuşatılmış/debi/akım anahtarları; bulunamazsa son sütun.
        q_col = next((c for c in df.columns if "toplam su" in c.lower()),
                 next((c for c in df.columns if any(k in c.lower()
                       for k in ("q smoothed", "smoothed", "yumuşat", "debi", "inflow", "discharge", "flow", "akım", "akim"))),
                 df.columns[-1]))
...
# ARAYÜZ
# =====================================================================
st.title("🌊 Almus Barajı Havzası — Günlük Akım Tahmin Modeli")
st.caption("Hibrit ML (otoregresif + hava/kar-temelli) • Günlük P/T/PET (Open-Meteo) + MODIS günlük SCF (GEE) • E14A018")

with st.sidebar:
    c1, c2 = st.columns([2, 1])
    c1.markdown(f"👤 **{st.session_state.username}**" + (" ⭐" if st.session_state.is_admin else ""))
    if c2.button("🚪 Çıkış", use_container_width=True):
        st.session_state.authenticated = False
        st.rerun()
    st.markdown("---")

# ---- Sidebar inputları (admin olmayana sadece Q₀ gösterilir) ----
flow_up = kmz_up = river_up = None
gee_project = os.environ.get("EARTHENGINE_PROJECT", "")
pt_mode = "Yükseklik bantları (kar-baskın) ⭐"
snow_dom = True; single_point = False
n_bands, n_points = 3, 16
auto_cal = True; manual_snow = None
peak_focus = True; split_year_tt = 2021

if st.session_state.is_admin:
    st.sidebar.header("📁 Girdiler")
    flow_up = st.sidebar.file_uploader("Günlük akım (.xlsx) — boşsa E14A018_daily_inflow_1999-2025.xlsx", type=["xlsx"])
    kmz_up = st.sidebar.file_uploader("Havza poligonu (.kmz/.kml) — boşsa watershed (5).kml", type=["kmz", "kml"])
    ...
    pt_mode = st.sidebar.radio("Nokta şeması",
        ["Yükseklik bantları (kar-baskın) ⭐", "Tek nokta — havza ortalaması", "Grid / Thiessen"], index=0,
        help="Almus büyük (~2364 km²) ve kar etkili bir havzadır; yükseklik bantları rakım gradyanını "
             "(kar birikimi/erimesi) yakaladığı için varsayılan. Küçük havzalarda tek nokta daha iyi olabilir — "
             "şemaları karşılaştırıp kör test NSE'sine göre seçin.")
    ...
    split_year_tt = st.sidebar.selectbox("Train/Test bölme yılı (test bu yıldan itibaren KÖR)",
                                         [2018, 2019, 2020, 2021, 2022, 2023], index=3)
...
# Almus varsayılanları: birim m³/s (debi); başlangıç 2010 (SCF çekimini sınırlamak için; boş=tümü 1999-2025)
flow_unit_factor = 1.0; flow_start = "2010-01-01"
_Q_KEYS = ("q smoothed", "smoothed", "yumuşat", "toplam su", "debi", "inflow", "discharge", "flow", "akım", "akim")
if flow_bytes is not None and st.session_state.is_admin:
    with st.sidebar.expander("📑 Akım dosyası formatı (generic)", expanded=False):
        try:
            ...
            _si = next((i for i, s in enumerate(_sheets) if s in ("Daily_Inflow", "Zaman Serisi")), 0)
            ...
            # Akım sütunu: 'Q smoothed' vb. anahtarları öne al; 'flag'i ASLA seçme; bulunamazsa son sütun
            _qi = next((i for i, c in enumerate(_cols) if "flag" not in c.lower()
                        and any(k in c.lower() for k in _Q_KEYS)), len(_cols) - 1)
            flow_q_col = st.selectbox("Akım sütunu", _cols, index=_qi)
            _units = list(UNIT_TO_M3S.keys())
            _ui = _units.index("m³/s (debi)") if "m³/s (debi)" in _units else 0
            flow_unit_factor = UNIT_TO_M3S[st.selectbox("Akım birimi", _units, index=_ui)]
            flow_start = st.text_input("Başlangıç tarihi (YYYY-MM-DD, boş=tümü)", value="2010-01-01").strip() or None
        except Exception as _e:
            st.error(f"Akım dosyası okunamadı: {_e}")

# Q₀ — her kullanıcıya gösterilir
Q0 = st.sidebar.number_input("Bugünkü Q₀ (m³/s) — canlı tahmin için", min_value=0.0, value=8.0, step=0.5)

# ---- Girdileri çöz (yükleme ya da varsayılan disk dosyaları) ----
kmz_src = kmz_up if kmz_up is not None else (DEFAULT_KMZ if DEFAULT_KMZ and os.path.exists(DEFAULT_KMZ) else None)

if flow_bytes is None or kmz_src is None:
    st.info("Başlamak için günlük akım Excel'i ve havza KMZ/KML'sini yükleyin "
            "(ya da bu klasöre `E14A018_daily_inflow_1999-2025.xlsx` ve `watershed (5).kml` koyun).")
    st.stop()
...
# ---- Hidrolojik modelden HARİÇ tutulacak akım yılları (anomali/şüpheli veri) ----
_avail_years = sorted(df_flow["Date"].dt.year.unique().tolist())
_default_excl = [2017] if 2017 in _avail_years else []
exclude_years = _default_excl
if st.session_state.is_admin:
    exclude_years = st.sidebar.multiselect(
        "🚫 Modelden hariç tutulacak akım yılları", _avail_years, default=_default_excl,
        help="Seçilen yıllar eğitim/doğrulamadan çıkarılır (akım hedefi düşer); hava serisi sürekli kalır "
             "(yuvarlanan/kar özellikleri boşluk boyunca ısınmaya devam eder). Varsayılan: 2017.")
if exclude_years:
    df_flow = df_flow[~df_flow["Date"].dt.year.isin(exclude_years)].reset_index(drop=True)
    if df_flow.empty: st.error("Tüm yıllar hariç tutuldu — en az bir yıl bırakın."); st.stop()
    st.sidebar.caption(f"Hariç: {', '.join(map(str, exclude_years))} ({len(_avail_years)-len(set(exclude_years) & set(_avail_years))} yıl kaldı).")
...
st.sidebar.download_button("📥 Üretilen noktalar (KMZ)", points_to_kmz_bytes(BANDS, basin_area),
                           f"almus_points_{BHASH}.kmz", "application/vnd.google-earth.kmz")
...
with tab1:
    ...
    if fc.empty:
        ...
        if not fcw.empty:
            ...
            st.caption(f"Havza ortalaması. Kırmızı: sıcaklık, mavi: yağış. Kesik çizgi = uzun dönem (gün-of-yıl) ortalaması, "
                       f"bant = tarihsel min–max (±7 gün penceresi, {flow_start:%Y}-{flow_end:%Y}).")
...
with tab3:
    ...
    if not fc.empty:
        ...
        st.download_button("📥 Tahmin (CSV)", show.to_csv(index=False).encode("utf-8"),
                           "almus_16gun_tahmin.csv", "text/csv")
    st.markdown("---")
    ...
    st.download_button("📥 Tarihsel (CSV)", hist_out.to_csv(index=False).encode("utf-8"),
                       "almus_tarihsel_model.csv", "text/csv")
